...
const Page = () => {
  ...
  const { mutate: sendMessage, isPending } = useMutation({
    mutationFn: async ({ text }: { text: string }) => {
      await client.apimessages.post({ sender: username, text }, { query: { roomId } })
...
const Page = () => {
  ...
  const { data: ttlData } = useQuery<{ ttl: number }>({
    queryKey: ["ttl", roomId],
    queryFn: async () => {
      const res = await fetch(`/api/room/ttl?roomId=${roomId}`)
      if (!res.ok) throw new Error('Failed to fetch TTL')
  ...
  const { mutate: destroyRoom } = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/room?roomId=${roomId}`, { method: 'DELETE' })
